        ...
        <div style={{
            ...
            <div style={{ display: 'grid', gap: '1rem', marginBottom: '1.5rem' }}>
                <div>
                    ...
                    <input aria-label="Car price" type="range" min="10000" max="60000" step="1000" value={carPrice}
                        onChange={(e) => {
                            const nextPrice = Number(e.target.value);
                            setCarPrice(nextPrice);
                            setDownPayment((current) => Math.min(current, nextPrice / 2));
                        }}
                ...
                <div>
                    ...
                    <input aria-label="Down payment" type="range" min="0" max={carPrice / 2} step="500" value={downPayment}
                ...
                <div>
                    ...
                    <input aria-label="Loan interest rate" type="range" min="0" max="15" step="0.5" value={loanRate}
                ...
                <div>
                    ...
                    <input aria-label="Lease monthly payment" type="range" min="150" max="600" step="25" value={leaseMonthly}
            ...
            <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '1rem', textAlign: 'center' }}>
                Illustration only. Cash includes a {investmentReturn}% assumed opportunity cost. Taxes, fees, maintenance,
                depreciation, lease due-at-signing costs, mileage charges, and resale value are not included.
            </p>
        </div>
    );
}

function ResultCard({ title, total, subtitle }) {
    return (
        <div style={{
            padding: '1rem',
            ...
            <div style={{ fontSize: '0.75rem', fontWeight: 'bold', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>
                {title}
            </div>
